...
echo admin | ssh -S admin@$iocage_01 sudo iocage list -lt | tee out/out-02.txt
...
ansible-playbook vbotka.freebsd.pb_iocage_ansible_clients.yml -i iocage.ini -t list -e debug=true | tee out/out-09.txt

echo admin | ssh -S admin@$iocage_01 sudo iocage list -l | tee out/out-05.txt

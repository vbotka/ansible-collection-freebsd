...
# Create template
# (cd ../202 && ansible-playbook -i iocage.ini -l iocage_04 vbotka.freebsd.pb_iocage_template.yml)

# Create swarm
ansible-playbook -i iocage.ini -t swarm -e swarm=true vbotka.freebsd.pb_iocage_ansible_clients.yml | tee out/out-11.txt
...
# Update repos
ansible-playbook -i iocage.ini vbotka.freebsd.pb_iocage_update_repos.yml | tee out/out-12.txt

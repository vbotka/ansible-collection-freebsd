...
#
# ssh admin@$build.example.com sudo tree /usr/local/etc/ssl/ | tee out/out-10.txt
# ssh admin@$build.example.com cat /usr/local/etc/poudriere.conf | tee out/out-11.txt
# ssh admin@$build.example.com tree /usr/local/etc/poudriere.d/pkglist | tee out/out-12.txt
# ssh admin@$build.example.com cat /usr/local/etc/poudriere.d/pkglist/arm/ansible | tee out/out-13.txt
# ssh admin@$build.example.com cat /usr/local/etc/poudriere.d/pkglist/arm/minimal | tee out/out-14.txt
# ssh admin@$build.example.com cat /usr/local/etc/poudriere.d/make.conf | tee out/out-15.txt
#

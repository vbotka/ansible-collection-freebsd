...
ansible-playbook -i hosts pb-logclient.yml | tee out/out-02.txt

# Stop and convert template.
ansible-playbook -i iocage.ini pb-logclient-stop-convert.yml | tee out/out-03.txt
...
# Test Log Server
ansible-playbook -i hosts -e debug=true pb-logclient-test.yml | tee out/out-08.txt
